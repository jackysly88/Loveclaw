...
function getSessionFilePath() {
  const os = require('os');
  return path.join(os.homedir(), '.openclaw', 'workspace', 'loveclaw', 'data', 'sessions.json');
}
 ...
 * 保存会话到文件
 */
function saveSessionsToFile(sessionsMap) {
  try {
    const filePath = getSessionFilePath();
...
function getUserSession(userId) {
  if (!userSessions.has(userId)) {
    ...
    const sessionsFile = loadSessionsFromFile();
    if (sessionsFile.has(userId)) {
      userSessions.set(userId, sessionsFile.get(userId));
    } else {
...
function saveUserSession(userId) {
  if (userSessions.has(userId)) {
    const sessionsFile = loadSessionsFromFile();
    sessionsFile.set(userId, userSessions.get(userId));
    saveSessionsToFile(sessionsFile);
  }
}
...
回复「开始报名」继续`
  ...
  switch (session.state) {
    case UserState.START:
      if (message === '开始报名' || message === '开始填写' || message === '报名') {
        session.state = UserState.NAME;
        return { text: STATE_QUESTIONS[UserState.NAME] };
      }
      // START 状态：用户任何输入都直接进入姓名流程（跳过"开始报名"步骤）
      session.data = {};
